...
<xsl:stylesheet version='2.0'
  ...
  <xsl:variable name="SOLR_HOME" as="xs:string">.</xsl:variable>
...
<p style="margin-top: 1em; margin-bottom: .5em;">You can further refine your query by:</p>
  ...
  <xsl:otherwise>
	...
	<xsl:template match="doc">
		...
		<xsl:choose>
			<xsl:when test="(arr|str)[@name='data_product_type'] = 'Product_Collection' or (arr|str)[@name='data_product_type']='Product_Bundle'">
				<xsl:choose>
