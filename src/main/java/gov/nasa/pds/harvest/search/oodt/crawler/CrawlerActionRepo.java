...
/**
 * A repository and reader for {@link CrawlerAction}s associated with Crawler lifecycle phases:
...
public class CrawlerActionRepo {
  ...
  public void loadActions(List<CrawlerAction> crawlerActions) {
    for (CrawlerAction action : crawlerActions) {
      for (String phase : action.phases) {
        switch (CrawlerActionPhases.getPhaseByName(phase)) {
          ...
          case POST_INGEST_FAILURE:
            ...
            break;
          default:
            throw new RuntimeException("Phase '" + phase + "' is not supported");
        }
      }
    }
  }

  public void loadActionsFromBeanFactory(ApplicationContext context, List<String> actionIds) {
    for (String actionId : actionIds) {
      CrawlerAction action = ((CrawlerAction) context.getBean(actionId, CrawlerAction.class));
