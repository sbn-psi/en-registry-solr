...
public class XMLUtils {
   ...
   * @param filename The filename to write the DOM document to.
   */
  ...
  public static List readMany(Element root, String elt, String encoding) {
    ...
    for (int i = 0; i < valueNodes.getLength(); i++) {
      ...
      try {
        value = URLDecoder.decode(DOMUtil.getSimpleElementText(valElem), encoding);
        values.add(value);
      } catch (Exception e) {
        LOG.log(Level.SEVERE, e.getMessage());
        LOG.log(Level.WARNING,
            "Error decoding tag: [" + elt + "]: val: [" + DOMUtil.getSimpleElementText(valElem)
  ...
  public static String read(Element root, String elt, String encoding) {
    ...
    try {
      ...
      LOG.log(Level.WARNING,
          "Error decoding " + elt + "from metadata. " + "Message: " + e.getMessage());
    }
    return value;
  }
  ...
  public static void addNode(Document doc, Node parent, String ns, String name, String text,
      Map NS_MAP) {
