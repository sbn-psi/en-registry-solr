...
/**
 * The DateTimeConverter class is necessary to convert PDS4-compliant date/time strings into
 * Solr-Compliant date/time Strings
 * 
 * The Solr DateField, which is necessary for performing time queries, is ISO 8601
 * standard-compliant format, while PDS4 uses a variable number of ASCII date/time formats.
 * 
 * @author jpadams
 * 
 */
public class PDSDateConvert {
  ...
  private static final String[] DATE_TIME_FORMATS = {"yyyyMMddHHmmss", "yyyy-MM-dd'T'HH:mm:ss",
      "yyyy-DDD'T'HH:mm:ss", "yyyy-MM-dd'T'HH:mm", "yyyy-MM-dd'T'HH"};
  ...
  /**
   * Fix too many values after decimal. When there are > 3 values after decimal for time value
   * SimpleDateFormat adds the value in the tenths place to seconds and moves the other values to
   * the left i.e ...:00.2456 -> ...:02.456
   * 
  ...
  /**
   * Parse the given date/time string using input format and date and return the resulting
  ...
  /**
   * Get a default time depending on the name of the field. "start" fields will receive an "early"
   * default, while remaining fields will receive a default in the future.
   * 
   * @param name
   * @return
   */
