...
public class DateUtils {
  ...
  public enum FormatType {
    UTC_FORMAT, LOCAL_FORMAT, TAI_FORMAT
  }
  ...
  static long[][] dateAndLeapSecs =
      {{0, 10}, {new GregorianCalendar(1972, GregorianCalendar.JULY, 1).getTimeInMillis(), 11},
  ...
  public static synchronized Calendar toLocalCustomFormatCalendar(String calString, String format)
  ...
  public static synchronized Calendar toCalendar(String calString, FormatType formatType)
    ...
    switch (formatType) {
      ...
      case TAI_FORMAT:
        cal.setTimeZone(
            createTaiTimeZone(Integer.parseInt(calString.substring(calString.length() - 2))));
  ...
  public static long getTimeInMillis(Calendar cal, Calendar epoch) throws CommonsException {
    ...
    /**
     * Fixes date conversion issues preventing tests passing in the UK but working elsewhere in the
     * world.
     */
    if (julianEpoch.getTimeZone().getID().equals("Europe/London")) {
      epochDiffInMilli = epoch.getTimeInMillis() - (julianEpoch.getTimeInMillis()
          + julianEpoch.getTimeZone().getOffset(julianEpoch.getTimeInMillis()));
    } else {
      epochDiffInMilli = epoch.getTimeInMillis() - julianEpoch.getTimeInMillis();
    }
    if (cal.getTimeZone().getID().equals("TAI")) {
  ...
  public static void main(String[] args) throws ParseException, CommonsException {
    ...
    System.out.println("Test Time: " + toString(
        toCalendar(toString(toTai(toCalendar("2008-01-20T16:29:55.000Z", FormatType.UTC_FORMAT))),
            FormatType.TAI_FORMAT)));
    System.out
        .println("Current Local Time: " + toString(curTime) + " " + curTime.getTimeInMillis());
    System.out.println("Current UTC Time: "
        + toString((curTime = toCalendar("2008-01-20T16:29:55.000Z", FormatType.UTC_FORMAT))) + " "
        + toString(getTimeInSecs(curTime, tai93epoch)));
    System.out.println("Current TAI Time: "
        + toString((curTime = toTai(toCalendar("2008-01-20T16:29:55.000Z", FormatType.UTC_FORMAT))))
        + " " + toString(getTimeInSecs(curTime, tai93epoch)));
